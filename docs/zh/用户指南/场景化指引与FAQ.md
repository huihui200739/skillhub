...
Team Skill（Swarm Skill）另须在 `SKILL.md` frontmatter 中设置 `kind: team-skill`（或 `swarm-skill`）及 `roles` 列表（至少 2 个角色），详见发布表单校验提示。
...
详细命令与 ClawHub 兼容说明见 [cli/README.md](../../../cli/README.md) 与 [ClawHub 兼容层 API](../接口文档/v1/ClawHub兼容层.md)。
...
完整错误模型见 [TeamSkillsHub API — 全局错误响应](../接口文档/v1/TeamSkillsHub.md#全局错误响应)。
