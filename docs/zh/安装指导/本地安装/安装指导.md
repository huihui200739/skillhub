...
本文用于说明如何在 **Windows** 上安装并启动 **marketplace**（SkillHub 市场服务）。下文默认仓库根目录为 `skillhub`（或你的实际克隆路径）。
...
- 若将 `MARKET_SKILL_REVIEW_ENABLED=true`：
  - Skill 发布后会先进入系统审查
  - 系统审查通过后再进入人工审核；系统审查不通过则直接发布失败
  - 必须同时配置完整的 `MARKET_SKILL_REVIEW_MODEL_*` 语义模型参数；缺失模型配置时 Skill 发布会被前置拒绝，不会创建待审版本
- 若保持 `MARKET_SKILL_REVIEW_ENABLED=false`：
  - Skill 发布后**不走系统审查**
  - 直接进入原有人工审核阶段
