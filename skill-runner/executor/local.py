...
# 产物文件预览单文件读取上限：超出只回前 N 字节并标 truncated
...
class LocalExecutor(SandboxExecutor):
    ...
    async def run_turn(
        ...
        # !cmd 调试入口：在工作目录里直接跑 shell 命令，不走 LLM。
        # 面向终端用户的确定性命令执行入口，默认关闭，生产必须 gate。
    ...
    async def _exec_local(
        ...
        """!cmd 调试：在 session 工作目录内直接跑一条命令（无 LLM、无 agent）。"""
        if not command:
            yield sse_event("error", code="empty_command", message="command empty")
            return
        # 历史调试简写："!cmd <foo>" 里 leading "cmd " 视为 UI 别名
    ...
    async def _build_team_agent(self, session: Session, work_dir: str) -> None:
        ...
        from openjiuwen.agent_teams.schema.deep_agent_spec import WorkspaceSpec  # noqa: PLC0415
        ...
        # default 模式无预定义名单：roles 仅作 leader 动态 spawn 用的 persona 模板，不预建成员，
        # 否则同一角色会"预建 + 动态 spawn"两份并存，产生冗余成员。
    ...
    def _make_sysop_spec(self, session: Session, oj: dict) -> Any:
        from openjiuwen.agent_teams.schema.deep_agent_spec import SysOperationSpec  # noqa: PLC0415
    ...
    # ------------------------------------------------------------------
    # Skill provisioning (local filesystem)
    # ------------------------------------------------------------------
    ...
    def _session_dirs(self, session: Session) -> tuple[str, str]:
        # skill 在 work 内：agent cwd=work_dir，相对路径 ./skill/ 即可读到
    ...
    def _make_work_config(oj: dict) -> Any:
        # 容器级放行：playground 要跑任意 skill 代码，shell/fs 在 pod 内不限制。
        # 真正的隔离边界是容器（非 root + drop caps + ephemeral）+ VPC 网络。
    ...
    # ------------------------------------------------------------------
    # Artifact files (workspace 产物，供前端「产物文件」面板读取)
    # ------------------------------------------------------------------
    async def list_files(self, session: Session) -> list[dict[str, Any]]:
        """列出 work 目录下 agent 产出的文件（排除输入用的 ./skill 子目录）。"""
        work_dir, _ = self._session_dirs(session)
        ...
        for root, dirs, files in os.walk(work_dir):
            ...
            # 顶层的 skill/ 是技能输入包、不是产物，整棵剪掉
            if rel_root == ".":
                dirs[:] = [d for d in dirs if d != "skill"]
            for fn in files:
    ...
    def _resolve_artifact_path(self, session: Session, rel_path: str) -> str:
        """把相对路径解析为 work 目录内的安全绝对路径；越界/指向 skill 输入则拒绝。"""
        work_dir, skill_dir = self._session_dirs(session)
        work_norm = os.path.normpath(work_dir)
        candidate = os.path.normpath(os.path.join(work_dir, rel_path))
        if candidate != work_norm and not candidate.startswith(work_norm + os.sep):
            raise ValueError("path escapes workspace")
        skill_norm = os.path.normpath(skill_dir)
        if candidate == skill_norm or candidate.startswith(skill_norm + os.sep):
            raise FileNotFoundError("skill input is not an artifact")
        return candidate

    async def read_file(self, session: Session, path: str) -> dict[str, Any]:
        """读单个产物文件；超限截断、二进制只报元信息。"""
        full = self._resolve_artifact_path(session, path)
        ...
        data = raw[:_MAX_FILE_READ_BYTES]
        norm_path = path.replace(os.sep, "/")
        try:
            content = data.decode("utf-8")
        except UnicodeDecodeError:
            return {"path": norm_path, "size": size, "binary": True,
                    "truncated": truncated, "content": ""}
        return {"path": norm_path, "size": size, "binary": False,
