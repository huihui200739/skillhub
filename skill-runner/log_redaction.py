...
# llm_proxy 签发的 pod token（"pod-" + token_urlsafe）
...
class SensitiveLogFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        try:
            ...
            # args 仅在 tuple（%-位置参数）时逐个处理；dict（%(name)s）原样放行，
        ...
        except Exception as exc:  # noqa: BLE001 - 脱敏失败绝不能阻断日志
...
def configure_sensitive_log_redaction() -> None:
    ...
    既挂到具体 logger（拦截直接 emit 的记录），也挂到 root 的所有 handler
    （拦截子 logger 如 skill_runner.k8s 传播上来、由 handler 处理的记录）。
    """
    global _CONFIGURED
    if _CONFIGURED:
        return
    ...
    # httpx/httpcore 的 INFO 会打完整 URL（含 query token），降到 WARNING
