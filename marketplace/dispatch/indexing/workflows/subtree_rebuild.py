...
from indexing.workflows.artifacts import ResolvedBuildConfig
from indexing.workflows.tree_ops import slug_term, unique_child_cid
...
def build_subtree_from_llm_groups(
    *,
    ...
    def append_group(parent_cid: str, group: dict[str, object], group_index: int) -> None:
        ...
        if len(skill_ids) > max_direct_leaf_children:
            raise ValueError(
                f"oversized LLM subtree group without child groups: "
                f"{branch_cid} has {len(skill_ids)} skills"
            )
