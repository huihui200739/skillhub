...
from indexing.io import normalize_item_paths
...
from indexing.scanners import get_scanner_class
    ...
    *,
    ...
    for index, raw_path in enumerate(item_paths):
        ...
        if is_s3_uri(raw_text) or is_http_uri(raw_text):
            ...
            resolved.append(
                ResolvedItemPath(source_path=raw_text, source_type=source_type, materialized_dir=item_dir)
            )
            continue
        ...
        if local_path.is_file() and local_path.suffix.lower() == ".zip":
            ...
            continue
        raise ValueError(
            f"Unsupported item path: {raw_text}. "
            "Only local dir/zip, s3://...zip, and http(s)://...zip are supported"
        )
