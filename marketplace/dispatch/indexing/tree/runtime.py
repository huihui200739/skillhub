...
from shared.rich_compat import BarColumn, Panel, Progress, SpinnerColumn, TaskProgressColumn, TextColumn
        ...
        for skill_id, payload in raw_profiles.items():
            ...
            profiles[normalized_id] = {
                "description": self._compact_profile_field(
                    ...
                    limit=self._skill_profile_description_limit,
                ),
                "select_when": self._compact_profile_field(
                    payload.get("select_when"),
                    limit=self._skill_profile_rule_limit,
                ),
    ...
    def _compose_skill_routing_description(
        ...
        *,
        ...
        if self._skill_profile_select_rules_enabled and select_when:
            parts.append(
                "Select when: "
                f"{self._compact_profile_field(select_when, limit=self._skill_profile_rule_limit)}"
            )
        if self._skill_profile_select_rules_enabled and dont_select_when:
            parts.append(
                "Don't select when: "
                f"{self._compact_profile_field(dont_select_when, limit=self._skill_profile_rule_limit)}"
            )
