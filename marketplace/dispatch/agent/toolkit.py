...
from indexing.io import load_tree_preset
...
from prompts import AGENTIC_RETRIEVAL_YAML, get_prompt
from retrieval.io.loading import CatalogRecord, LoadedRetrieverIndex, load_retriever_index
from retrieval.tree.subtree import DefaultCurrentSubtreeProvider
from retrieval.tree.types import ProgressiveRetrieverConfig, SearchCursor
...
class AgenticSkillRetrievalToolkit:
    ...
    def build_index(
        ...
        *,
        ...
        with TemporaryDirectory(prefix="dispatch-skill-index-") as tmp:
            tmp_root = Path(tmp)
            ...
            try:
                ...
                _emit(progress_callback, self.load_index_status())
                try:
                    build_check(stage="build")
                except SkillIndexBuildCancelled:
                    return self._cancelled_result(build_id=build_id, started=started, fingerprint=fingerprint)
                config = _to_dispatch_build_config(build_cfg, llm_cfg)
                if operation == "build":
                    _run_index_builder(
                        operation=operation,
        ...
        *,
        ...
        with _TASKS_LOCK:
            ...
            thread = threading.Thread(
                ...
                daemon=True,
                name=f"dispatch-skill-index-{build_id}",
            )
    ...
    def branch_explore(
        ...
        *,
        ...
        config = replace(
            ...
            max_exposure_depth=(
                max_exposure_depth
                if max_exposure_depth is not None
                else base_config.max_exposure_depth
            ),
        )
    ...
    def _failed_build_result(
        ...
        *,
        ...
        previous_index_available = (
            _is_complete_index(self.index_dir)
            if previous_index_available is None
            else previous_index_available
        )
...
def _to_dispatch_build_config(config: SkillIndexBuildConfig, llm: LLMConfig) -> BuildConfig:
    return BuildConfig(
...
def _is_complete_index(index_dir: Path) -> bool:
    return all(
        ...
        for filename in (TREE_INDEX_FILENAME, CATALOG_FILENAME, MANIFEST_FILENAME)
    )
