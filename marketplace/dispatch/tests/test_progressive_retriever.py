...
from retrieval.llm.base.tokenization import CandidateCodeTokenizer
...
class ProgressiveRetrieverTests(unittest.TestCase):
    ...
    def test_single_forward_logit_selection_selects_top_visible_candidate_when_enabled(self) -> None:
        ...
        scoring_requested = next(
            ...
            if event.event_type == "fragment_logit_selection_requested"
        )
        scoring_completed = next(
            event for event in result.trace.events
            if event.event_type == "fragment_logit_selection_completed"
        )
