...
from indexing.tree.schema import DynamicTreeConfig


class _FakeGroupingBuilder:
    def __init__(self) -> None:
        self._deterministic_prompts = True
        self._discovery_seed = 123
        self.config = DynamicTreeConfig(branching_factor=3)
        self._manager_config = SimpleNamespace(build=SimpleNamespace(classify_batch_cap=2))
        self.max_workers = 4
        self.auto_batch_size = 2
    ...
    def _call_llm_json(self, prompt: str, **kwargs) -> dict:
        del kwargs
