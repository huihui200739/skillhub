...
from retrieval.tree.engine import RecursiveSearchEngine
from retrieval.tree.expand import DefaultTargetExpander
from retrieval.tree.flat import FlatRetriever
from retrieval.tree.reduce import DefaultBranchReducer
from retrieval.tree.render import DefaultSubtreeRenderer
...
from retrieval.tree.select import DefaultTopKSelector
from retrieval.tree.select.selection import (
    GenerateFragmentSelector,
    LogitSelectionFragmentSelector,
)
from retrieval.tree.subtree import DefaultCurrentSubtreeProvider
from retrieval.tree.subtree.roots import (
...
from retrieval.llm.base.types import CandidateScore
from retrieval.tree.types import (
    ChildSearchCursor,
    CurrentSubtree,
    ExpansionPlan,
    NodeSearchResult,
    ...
    def test_dedup_candidates_removes_duplicates(self) -> None:
        ...
        child = (
            NodeSearchResult(
                ...
                RetrieverCandidate(
                    rank=1, item_id="b1",
                    payload="p.shared", branch_path=("b",), label="B",
                ),
            )
            ),
        )
        result = reducer.reduce_branch_results(
            cursor=SearchCursor(
                ...
                depth=0, branch_path=("root",), top_k=10,
            ),
            local_leaves=local,
            child_results=child,
        )
...
class ProtocolRaiseTests(unittest.TestCase):
    ...
    def test_topk_selector_build_protocol_raises(self) -> None:
        class DelegatingSelector(TopKSelector):
            ...
            def select_topk(self, *, model, cursor, query_messages,
                            subtree, prompt, trace):
                return super().select_topk(
                    model=model, cursor=cursor,
                    query_messages=query_messages, subtree=subtree,
    ...
    def test_topk_selector_select_raises(self) -> None:
        ...
        with self.assertRaises(NotImplementedError):
            obj.select_topk(
                model="", cursor=MagicMock(),
                query_messages=[], subtree=MagicMock(),
                prompt=MagicMock(), trace=MagicMock(),
            )
    ...
    def test_branch_reducer_protocol_raises(self) -> None:
        class DelegatingReducer(BranchReducer):
            def reduce_branch_results(
                self, *, cursor, local_leaves, child_results
            ):
                return super().reduce_branch_results(
                    cursor=cursor, local_leaves=local_leaves,
...
class ExpandEdgeCaseTests(unittest.TestCase):
    def test_resolve_branch_top_k_with_zero_branch_count(self) -> None:
        expander = DefaultTargetExpander(
            config=ProgressiveRetrieverConfig(branch_candidate_slack=0)
        )
        result = getattr(expander, '_resolve_branch_top_k')(
            top_k=5, branch_count=0
        )
        self.assertEqual(result, 5)

    def test_resolve_branch_top_k_with_slack(self) -> None:
        expander = DefaultTargetExpander(
            config=ProgressiveRetrieverConfig(branch_candidate_slack=3)
        )
        result = getattr(expander, '_resolve_branch_top_k')(
            top_k=5, branch_count=2
        )
...
class EngineParallelBranchTests(unittest.TestCase):
    def test_engine_parallel_branches_with_multiple_children(self) -> None:
        ...
        root_cursor = SearchCursor(
            ...
            depth=0, branch_path=("root",), top_k=2,
        )

        root_resolution = SelectableResolution(
            code="Alpha", canonical_id="alpha", display_name="Alpha", label="Alpha",
            description="", is_terminal=True, branch_path=("root", "alpha"), item=leaf_item, node=None,
        )
        branch_a_resolution = SelectableResolution(
            code="ChildA", canonical_id="child_a", display_name="ChildA", label="ChildA",
            description="", is_terminal=False, branch_path=("root", "child_a"), item=None, node=child_a_node,
        )
        branch_b_resolution = SelectableResolution(
            code="ChildB", canonical_id="child_b", display_name="ChildB", label="ChildB",
            description="", is_terminal=False, branch_path=("root", "child_b"), item=None, node=child_b_node,
        )

        root_fragment = ExposedFragment(
            root=ExposedNode(canonical_id="root", label="Root", description="", is_selectable=False, children=()),
            rendered_tree="- Alpha\n- ChildA\n- ChildB",
            compact_codes_enabled=False, code_width=0,
        ...
        provider = _StubProvider({
            "root": CurrentSubtree(
                cursor=root_cursor, fragment=root_fragment,
                selectable_targets=(
        ...
        expander = _StubExpander({
            ...
            "child_a": ExpansionPlan(leaf_results=(
                RetrieverCandidate(rank=1, item_id="ia", payload="p.a", branch_path=("root", "child_a"), label="A"),
            ), child_cursors=()),
            "child_b": ExpansionPlan(leaf_results=(
                RetrieverCandidate(rank=1, item_id="ib", payload="p.b", branch_path=("root", "child_b"), label="B"),
            ), child_cursors=()),
        })
        reducer = _StubReducer()
        engine = RecursiveSearchEngine(
            subtree_provider=provider, renderer=renderer, selector=selector,
            expander=expander, reducer=reducer, enable_parallel_branches=True,
        )
...
class EngineAbstainTests(unittest.TestCase):
    def test_engine_abstain_selection_returns_empty_candidates(self) -> None:
        ...
        alt_resolution = SelectableResolution(
            code="Alt", canonical_id="alt", display_name="Alt", label="Alt",
            description="", is_terminal=True, branch_path=("root", "alt"),
            item=_leaf_item("alt.item", "worker.alt", "Alt"), node=None,
        )
        ...
        provider = _StubProvider({
            "root": CurrentSubtree(
                cursor=root_cursor, fragment=alt_fragment,
                selectable_targets=(
        ...
        engine = RecursiveSearchEngine(
            subtree_provider=provider, renderer=renderer, selector=selector,
            expander=expander, reducer=reducer, enable_parallel_branches=False,
        )
...
class EngineEmptySelectableTests(unittest.TestCase):
    def test_engine_empty_selectable_targets_returns_empty(self) -> None:
        ...
        empty_fragment = ExposedFragment(
            root=ExposedNode(canonical_id="root", label="Root", description="", is_selectable=False, children=()),
            rendered_tree="", compact_codes_enabled=False, code_width=0,
            candidate_codes=(), selectable_nodes=(), code_to_resolution={},
        )
        provider = _StubProvider({
...
class LogitSelectionFallbackTests(unittest.TestCase):
    def test_abstain_fallback_returns_zero_and_empty_list(self) -> None:
        ...
        output, selected = selector.select(
            model="m", query_messages=[{"role": "user", "content": "q"}],
            node=node, depth=0, top_k=1, trace=RetrieverTrace(), fragment=fragment,
        )
    ...
    def test_candidate_count_exceeds_limit_triggers_fallback(self) -> None:
        ...
        r1 = SelectableResolution(
            code="Q1", canonical_id="x1", display_name="Q1",
            label="Q1", description="", is_terminal=True,
            branch_path=("r", "x1"),
            item=_leaf_item("i1", "p1", "L1"), node=None,
        )
        r2 = SelectableResolution(
            code="Q2", canonical_id="x2", display_name="Q2",
            label="Q2", description="", is_terminal=True,
            branch_path=("r", "x2"),
            item=_leaf_item("i2", "p2", "L2"), node=None,
        )
        fragment = ExposedFragment(
            root=ExposedNode(canonical_id="root", label="Root", description="", is_selectable=False, children=()),
            rendered_tree="- Q1\n- Q2", compact_codes_enabled=True, code_width=2,
            candidate_codes=("Q1", "Q2"), selectable_nodes=(), code_to_resolution={"Q1": r1, "Q2": r2},
        )
        node = RetrieverNode(node_id="n", label="N")
    ...
    def test_min_probability_filters_low_probability_scores(self) -> None:
        ...
        from retrieval.llm.base.scoring import build_candidate_scoring_result
        ...
        output, selected = selector.select(
            model="m", query_messages=[{"role": "user", "content": "q"}],
            node=node, depth=0, top_k=2, trace=RetrieverTrace(), fragment=fragment,
        )
...
class FlatRetrieverTests(unittest.TestCase):
    def _make_flat_retriever(self, llm_output: str) -> FlatRetriever:
        ...
        from retrieval.llm import ProgressiveLLMClient, LLMClientCapabilities, GenerationConfig
        ...
        class _StubLLM(ProgressiveLLMClient):
            ...
            def complete(
                self, model, messages, *, max_tokens=None,
                stop_sequences=None, generation_config=None,
                n=1, request_timeout=None,
            ):
