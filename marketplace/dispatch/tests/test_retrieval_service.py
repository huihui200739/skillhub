"""Unit tests for the refactored retrieval.service API."""
...
from models.retrieval import RetrieverChoice, RetrieverNode, RetrieverTraceEvent
...
from retrieval.service.defaults import serialize_hit_summary, serialize_trace_event
...
class ModelsTests(unittest.TestCase):
    ...
    def test_empty_generation_mode_defaults_to_generate(self) -> None:
        runtime = runtime_retriever_config_from_config(
            RetrieverConfig(generation_config=GenerationConfig(mode=""))  # type: ignore[arg-type]
        )
...
class RetrieverInstanceTests(unittest.TestCase):
    ...
    def test_from_index_loads_index_and_coerces_openai_client(self) -> None:
        ...
        with patch("retrieval.service.retriever.load_retriever_index", return_value=fake_index):
            with patch("retrieval.service.retriever._coerce_llm_client", return_value=llm) as coerce:
