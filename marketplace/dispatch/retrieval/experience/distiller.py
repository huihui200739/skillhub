...
from prompts import EXPERIENCE_YAML, get_prompt
...
class TraceDistiller:
    ...
    def __init__(
        ...
        *,
        ...
        if llm_client is not None and llm_model:
            ...
            self._llm = coerce_generation_client(llm_client)
        else:
            self._llm = None
    ...
    def run(self, clusters: list[ClusteredQuery]) -> list[DistilledPattern]:
        ...
        with ThreadPoolExecutor(max_workers=self._max_workers) as executor:
            ...
            for i, cluster in enumerate(clusters):
                LOGGER.info(
                    "TraceDistiller: submitting cluster %d (id=%d, centroid='%s')",
                    i, cluster.cluster_id, cluster.centroid_query,
                )
            ...
            for future in as_completed(future_to_idx):
                ...
                try:
                    ...
                    LOGGER.error(
                        "TraceDistiller: failed cluster %d (id=%d): %s",
    ...
    def _distill_one(self, cluster: ClusteredQuery) -> DistilledPattern | None:
        ...
        if self._llm and self._llm_model:
            try:
                ...
                if resp:
                    pattern_description = self._parse_response(resp[0], cluster.cluster_id)
            except Exception as exc:
                LOGGER.debug(
    ...
    def _compute_metrics(
        ...
        avg_success = (
            sum(t.token_cost for t in success_traces) / len(success_traces)
            if success_traces
            else 0.0
        )
        avg_failure = (
            sum(t.token_cost for t in failure_traces) / len(failure_traces)
            if failure_traces
            else 0.0
        )
    ...
    def _build_prompt(
        ...
        return get_prompt(EXPERIENCE_YAML, "distill_pattern").format(
            cluster_id=cluster.cluster_id,
