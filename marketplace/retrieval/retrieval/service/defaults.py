...
from models.retrieval import FinderTraceEvent
from .models import RetrievalMethod


def normalize_method(method: str | RetrievalMethod | None) -> str:
    normalized = str(
        method.value if isinstance(
            method, RetrievalMethod) else (
            method or "auto")).strip().lower() or "auto"
    if normalized in {"auto", "bm25", "embedding", "progressive"}:
        return normalized
    return "auto"


def serialize_trace_event(event: FinderTraceEvent) -> Dict[str, object]:
    return {
