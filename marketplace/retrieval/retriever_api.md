...
index_dir = Demo.IndexBuilder.build(
...
config = Demo.BuildConfig(
    method=Demo.BuildMethod.ALL,
...
```python
config = Demo.BuildConfig(
    method=Demo.BuildMethod.BM25 | Demo.BuildMethod.EMBEDDING,
    embedding_openai_client=embedding_client,
    embedding_model="text-embedding-3-large",
)

index_dir = Demo.IndexBuilder.add(
...
retriever = Demo.Retriever.from_index("/tmp/skill-index")

config = Demo.SearchConfig(
    top_k=5,
    method=Demo.RetrievalMethod.BM25,
)
...
retriever = Demo.Retriever.from_index(
    "/tmp/skill-index",
    embedding_openai_client=embedding_client,
    embedding_model="text-embedding-3-large",
)
...
config = Demo.SearchConfig(
    top_k=5,
    method=Demo.RetrievalMethod.EMBEDDING,
)
...
config = Demo.SearchConfig(
    top_k=10,
    method=Demo.RetrievalMethod.AUTO,
    llm_top_k=3,
)
