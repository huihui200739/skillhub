...
class PluginScanner(BaseScanner):
    ...
    def _plugin_metadata_candidates(cls, path: Path) -> tuple[Path, ...]:
        ...
        for parent in (path / ".codex-plugin", path):
    ...
    def scan_item_dir(self, item_dir: Path) -> ScannedItem | None:
        ...
        plugin_file = next(
            (candidate for candidate in self._plugin_metadata_candidates(item_root) if candidate.exists()),
            None,
        )
    ...
    def _parse_simple_yaml_payload(text: str) -> dict[str, Any]:
        ...
        for raw_line in text.splitlines():
            ...
            if indent == 0:
                ...
                continue
            if active_section and PluginScanner._is_nested_mapping_line(indent=indent, line=line):
    ...
    @staticmethod
    def _is_nested_mapping_line(*, indent: int, line: str) -> bool:
        return indent >= 2 and ":" in line and not line.startswith("- ")
