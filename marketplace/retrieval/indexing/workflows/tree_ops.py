...
def merge_added_skills_into_tree(*, nodes: Sequence[object], added_skills: Dict[str, dict]) -> List[Dict[str, object]]:
    ...
    branch_token_cache = {
        str(node.get("cid") or ""): text_tokens(
            str(node.get("cid") or ""),
    ...
    for worker_id, skill in sorted(added_skills.items()):
        parent_node_cid = choose_parent_branch_for_skill(
            skill=skill,
            branch_nodes=branch_nodes,
            branch_token_cache=branch_token_cache,
        )
        cid = unique_child_cid(
            parent_node_cid=parent_node_cid,
...
def choose_parent_branch_for_skill(
        *, skill: dict, branch_nodes: Sequence[Dict[str, object]], branch_token_cache: Dict[str, set[str]]) -> str:
    if not branch_nodes:
        return ""
...
def prune_deleted_skills_from_tree(nodes: Sequence[object], *, removed_worker_ids: set[str]) -> List[Dict[str, object]]:
    ...
    while True:
        ...
        for node in normalized:
            ...
            cid = str(node.get("cid") or "")
            if node_type != "branch" or child_counts.get(cid, 0) > 0:
                pruned.append(node)
...
def align_leaf_nodes_with_catalog(
        nodes: Sequence[object], records_by_worker: Dict[str, CatalogRecord]) -> List[Dict[str, object]]:
...
def build_catalog_records_from_existing(
        *, nodes: Sequence[object], records_by_worker: Dict[str, CatalogRecord]) -> List[CatalogRecord]:
...
def enrich_branch_descriptions(nodes: Sequence[object], *,
                               catalog_records: Sequence[CatalogRecord]) -> List[Dict[str, object]]:
...
def build_branch_description(*, cid: str, existing_description: str, descendants: Sequence[CatalogRecord]) -> str:
    ...
    parts.append(
        ...
        "; ".join(
            format_catalog_record_snippet(record) for record in samples))
...
def get_parent_cid(cid: str) -> str:
    return cid.rsplit(".", 1)[0] if "." in cid else ""


def unique_child_cid(*, parent_node_cid: str, segment: str, used: set[str]) -> str:
    candidate = join_cid(parent_node_cid, segment)
    if candidate not in used:
        return candidate
    index = 2
    while True:
        candidate = join_cid(parent_node_cid, f"{segment}-{index}")
        if candidate not in used:
            return candidate
        index += 1
