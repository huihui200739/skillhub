...
class TreeRepairEngine:
    ...
    def postprocess_tree(self, root: TreeNode, verbose: bool = False) -> None:
        ...
        for repair_pass in range(1, self._builder.postprocess_max_passes + 1):
    ...
    def rebalance_child_assignments(self, node: TreeNode, verbose: bool = False) -> int:
        ...
        if len(node.children) < 2:
            return 0
        groups = {child.id: {"name": child.name, "description": child.description} for child in node.children}
        if len(groups) < 2:
            return 0
        ...
        if len(skill_entries) < builder.postprocess_min_skills:
            return 0

        assignments = builder.classify_skills(skill_entries, groups, verbose=False)
        assignments = builder.validate_and_recover(skill_entries, groups, assignments, verbose=False)
        ...
        for skill_id, target_child_id in moves:
            ...
            builder.insert_skill_into_subtree(target_child, skill_data)

        removed_empty = builder.prune_empty_children(node)
        reassigned_tiny = builder.repair_small_leaf_children(node)
        if removed_empty or reassigned_tiny:
            builder.prune_empty_children(node)
            if len(node.children) >= 2:
                builder.rewrite_node_label_after_singleton(
                    node,
                    builder.existing_child_groups(node.children),
                    verbose=verbose,
                )
    ...
    def collect_subtree_skill_locations(self, node: TreeNode) -> list[tuple[TreeNode, dict]]:
        if node.is_leaf:
            return [(node, self._builder.skill_to_data(skill)) for skill in node.skills]
    ...
    def normalize_to_equivalence_groups(self, root: TreeNode, verbose: bool = False) -> None:
        ...
        if verbose and split_count > 0:
            console.print(
                f"[dim]  Equivalence regrouping updated {split_count} second-leaf "
                f"nodes under '{root.id}'[/dim]"
            )
    ...
    def split_second_leaf_node_into_equiv_groups(
        ...
        groups = builder.discover_equivalence_groups(second_leaf_node, leaf_children, verbose=verbose)
        if not groups:
            return [second_leaf_node]
        normalized_groups = builder.normalize_equivalence_groups(leaf_children, groups)
        ...
        for idx, group in enumerate(normalized_groups, start=1):
            base_id = builder.build_equivalence_group_id(
        ...
        if verbose:
            console.print(
                f"[dim]  Split '{second_leaf_node.id}' into "
                f"{len(replacement_nodes)} equivalence groups[/dim]"
            )
