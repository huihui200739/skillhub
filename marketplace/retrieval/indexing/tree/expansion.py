...
class TreeExpansionEngine:
    ...
    def process_node(
        ...
        *,
        ...
        if depth == 0 and node.id == "root":
            grouped = self._classify_root(skills, verbose=verbose)
        else:
            grouped = builder.split_skills(skills, parent_context, verbose)
    ...
    def _emit_leaf(self, node: TreeNode, skills: list[dict], *, verbose: bool) -> None:
        self._builder.assign_skills_to_leaf(node, skills)
        if verbose:
    ...
    def _classify_root(self, skills: list[dict], *, verbose: bool) -> dict:
        builder = self._builder
        root_groups = self.root_group_definitions()
        if verbose:
            console.print(f"[cyan]Routing {len(skills)} skills into root categories[/cyan]")
        initial = builder.classify_skills(skills, root_groups, verbose)
        repaired = builder.validate_and_recover(skills, root_groups, initial, verbose)
        return builder.build_groups_from_assignments(root_groups, repaired)
    ...
    def root_group_definitions(self) -> dict[str, dict[str, str]]:
        categories = self._builder.config.root_categories or FIXED_ROOT_CATEGORIES
        root_groups: dict[str, dict[str, str]] = {}
    ...
    def build_children_from_groups(
        ...
        for group_id, group_data in groups.items():
            ...
            if not child_skills:
                continue
            if len(child_skills) < 2 and not builder.equiv_allow_singleton_groups:
            ...
            children_to_process.append(ChildGroup(node=child_node, skills=child_skills))
        ...
        if skill_map:
            if children_to_process:
                ...
                builder.assign_skills_to_leaf(node, skills)
                return []
    ...
    def reassign_skills_to_children(
        ...
        assignments = builder.classify_skills_single(
            builder.sorted_skills(unassigned_skills),
            groups,
            verbose=False,
            is_retry=True,
        )
    ...
    def rewrite_node_label_after_singleton(
        ...
        result = builder.call_llm_json(prompt)
    ...
    def existing_child_groups(self, children: list[TreeNode]) -> list[ChildGroup]:
        builder = self._builder
        return [
            ChildGroup(node=child, skills=builder.collect_subtree_skill_dicts(child))
